...
import { tenantApi, type TenantInfo } from '@api/tenant/tenant'
    ...
    <TenantLayout title={t('tenantInfoPage.layoutTitle')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 flex items-center justify-between gap-3">
          <span>{t('tenantInfoPage.notice.readOnlySettings')}</span>
          <Link to={ROUTES.tenant.settings} className="text-blue-700 underline hover:text-blue-900">
            {t('tenantInfoPage.notice.openSettings')}
          </Link>
        </div>
              ...
              <div className="px-6 py-6">
                <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <dd className="mt-1">
                      <PBadge variant={getStatusVariant(tenantInfo.status) as any} icon={<CheckCircleIcon className="h-3 w-3" />}>
            ...
            <PCard className="rounded-xl p-0 shadow-sm">
              ...
              <div className="px-6 py-6">
                <div className="space-y-4">
                  <div>
                    <label className="mb-2 block text-sm font-medium text-gray-700">{t('tenantInfoPage.userAccessLinks.joinPage')}</label>
                    <div className="flex items-center space-x-2">
                      <PInput type="text" readOnly value={getJoinUrl()} className="flex-1 bg-gray-50 font-mono text-gray-600" />
                      <PButton type="button" variant="secondary" onClick={() => copyToClipboard(getJoinUrl(), 'join')} title={t('tenantInfoPage.actions.copyLink')}>
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-medium text-gray-700">{t('tenantInfoPage.userAccessLinks.loginPage')}</label>
                    <div className="flex items-center space-x-2">
                      <PInput type="text" readOnly value={getLoginUrl()} className="flex-1 bg-gray-50 font-mono text-gray-600" />
                      <PButton type="button" variant="secondary" onClick={() => copyToClipboard(getLoginUrl(), 'login')} title={t('tenantInfoPage.actions.copyLink')}>
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-medium text-gray-700">{t('tenantInfoPage.userAccessLinks.registerPage')}</label>
                    <div className="flex items-center space-x-2">
                      <PInput type="text" readOnly value={getRegisterUrl()} className="flex-1 bg-gray-50 font-mono text-gray-600" />
                      <PButton type="button" variant="secondary" onClick={() => copyToClipboard(getRegisterUrl(), 'register')} title={t('tenantInfoPage.actions.copyLink')}>
              ...
              <div className="px-6 py-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-semibold text-blue-600">{tenantInfo.stats.total_users}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-semibold text-green-600">{tenantInfo.stats.total_apps}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-semibold text-emerald-600">{tenantInfo.stats.active_apps}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-semibold text-indigo-600">{tenantInfo.stats.total_clients}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-semibold text-yellow-600">{tenantInfo.stats.active_tokens}</span>
                  </div>
                </div>
              </div>
            </PCard>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${Math.min((tenantInfo.stats.total_apps / tenantInfo.quota.max_apps) * 100, 100)}%` }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${Math.min((tenantInfo.stats.total_users / tenantInfo.quota.max_users) * 100, 100)}%` }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-gray-200">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm text-gray-500">{tenantInfo.quota.max_tokens_per_hour.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
