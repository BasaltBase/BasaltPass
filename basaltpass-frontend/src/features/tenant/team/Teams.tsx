...
export default function TenantTeamsPage() {
  ...
  const changeRole = async (m: any, role: string) => {
    if (!memberTeam) return
    await tenantTeamApi.updateMemberRole(memberTeam.id, m.user_id, role)
    reloadMembers()
  }
  ...
  return (
    <TenantLayout title="团队管理" actions={<PButton size="sm" leftIcon={<PlusIcon className="h-4 w-4" />} onClick={() => setShowCreate(true)}>新建团队</PButton>}>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          <div className="w-64">
            <PInput placeholder="搜索团队名称/描述" value={keyword} onChange={(e) => {
              setPage(1)
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 overflow-auto py-10">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <PButton size="sm" variant="ghost" onClick={() => {
                setMemberTeam(null)
                setMembers([])
              }}>关闭</PButton>
            </div>
