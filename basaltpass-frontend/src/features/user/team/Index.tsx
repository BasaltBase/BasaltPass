...
const TeamIndex: React.FC = () => {
  ...
  const loadTeams = async () => {
    try {
      ...
      setError(err.response?.data?.message || t('pages.teamIndex.errors.loadFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error) {
    return (
      <Layout>
        <PAlert variant="error" title={t('pages.teamIndex.errors.title')} message={error} />
      </Layout>
    );
  }
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <PPageHeader title={t('pages.teamIndex.title')} description={t('pages.teamIndex.description')} />
          <div className="flex space-x-3">
            <Link to={ROUTES.user.invitationsInbox}>
              <PButton variant="secondary">
                ...
                {t('pages.teamIndex.actions.invitationInbox')}
              </PButton>
            </Link>
            <Link to={ROUTES.user.teamsCreate}>
              <PButton variant="primary">
                ...
                {t('pages.teamIndex.actions.createTeam')}
              </PButton>
            </Link>
          </div>
        </div>
        ...
        {teams.length === 0 ? (
          <PCard className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">{t('pages.teamIndex.empty.title')}</h3>
            <p className="mt-1 text-sm text-gray-500">{t('pages.teamIndex.empty.description')}</p>
            <div className="mt-6">
              <Link to={ROUTES.user.teamsCreate}>
                <PButton variant="primary">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {teams.map((team) => (
              <PCard key={team.team_id} variant="bordered" hoverable className="p-6">
                ...
                <div className="mt-4 text-sm text-gray-500">
                  {t('pages.teamIndex.joinedAt', { date: new Date(team.joined_at).toLocaleDateString(locale) })}
                </div>
                <div className="mt-6 flex space-x-3">
                  <Link to={`/teams/${team.team_id}`} className="flex-1">
                    <PButton variant="secondary" className="w-full">{t('pages.teamIndex.actions.viewDetail')}</PButton>
                  </Link>
                  {(team.role === 'owner' || team.role === 'admin') && (
                    <Link to={`/teams/${team.team_id}/members`} className="flex-1">
                      <PButton variant="ghost" className="w-full">{t('pages.teamIndex.actions.manageMembers')}</PButton>
                    </Link>
                  )}
                </div>
              </PCard>
